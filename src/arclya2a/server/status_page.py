...
<head>
  ...
  <style>
    ...
    body {{ max-width: 48rem; margin: 2rem auto; padding: 0 1.25rem; }}
    h1 {{ font-size: 1.5rem; margin-bottom: 0.25rem; }}
    .badge {{ display: inline-block; padding: .2rem .65rem; border-radius: 999px; font-size: .85rem; }}
    .badge.ok {{ background: #dcfce7; color: #166534; }}
    .badge.warn {{ background: #fef3c7; color: #92400e; }}
    section {{ margin: 1.25rem 0; }}
    dl {{ display: grid; grid-template-columns: 11rem 1fr; gap: .35rem .75rem; margin: 0; }}
    dt {{ color: #666; }}
    a {{ color: #2563eb; }}
...
<body>
  ...
  <section>
    ...
    <dl>
      ...
      <dt>Checked at</dt><dd><code>{snapshot.get("checked_at", "—")}</code></dd>
